...
class DsDialogFrameABC(NameCheckDialogFrame):
    ...
    def __init__(self, parent_dialog, dialog_title, exists_ds_name_tuple, data_id=None, **kwargs):
        # 数据源内容信息布局
        self.ds_info_layout = ...
        # 框架布局，分四部分，第一部分为标题部分，第二部分为数据源名称表单部分，第三部分为数据源内容信息部分、第四部分为按钮部分
        super().__init__(parent_dialog, dialog_title, exists_ds_name_tuple, data_id,
                         data_id is not None, **kwargs)
