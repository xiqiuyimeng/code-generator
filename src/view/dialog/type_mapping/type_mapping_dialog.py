...
from src.view.list_widget.col_type_list_widget import ColTypeStackedListWidget
...
class TypeMappingDetailDialog(NameCheckDialog):
    add_type_mapping_signal = pyqtSignal(TypeMapping)
    edit_type_mapping_signal = pyqtSignal(TypeMapping)
    ...
    def __init__(self, screen_rect, type_mapping_names, type_mapping_id=None):
        ...
        # 左侧列表项控件
        self.list_widget: ColTypeStackedListWidget = ...
        ...
        # 通过是否存在类型映射id来判断是否需要读取数据库内容，如果存在则为编辑，应该读取数据库
        super().__init__(screen_rect, TYPE_MAPPING_TITLE, type_mapping_names,
                         type_mapping_id, type_mapping_id is not None)
    ...
    def setup_content_ui(self):
        ...
        self.list_widget = ColTypeStackedListWidget(self.frame)
    ...
    def add_post_process(self):
        self.add_type_mapping_signal.emit(self.new_dialog_data)
        self.close()
