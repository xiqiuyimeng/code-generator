...
from PyQt5.QtWidgets import QLabel, QWidget, QVBoxLayout, QPushButton, QHBoxLayout, QSplitter, QFrame, QListWidgetItem

from src.constant.template_dialog_constant import TEMPLATE_TITLE, TEMPLATE_INFO_TEXT, TEMPLATE_FILE_TEXT, \
    TEMPLATE_CONFIG_TEXT, TEMPLATE_NAME, TEMPLATE_DESC, READ_TEMPLATE_BOX_TITLE, ADD_TEMPLATE_BOX_TITLE, \
    EDIT_TEMPLATE_BOX_TITLE, ADD_FILE_BTN_TEXT, CREATE_FILE_TITLE, LOCATE_FILE_BTN_TEXT
...
from src.view.list_widget.list_item_func import set_template_file_data, get_template_file_data
...
class TemplateDetailDialog(CustomStackedWidgetDialog):
    add_data_signal = pyqtSignal(Template)
    edit_data_signal = pyqtSignal(Template)
    ...
    def __init__(self, screen_rect, template_names, template_id=None):
        ...
        super().__init__(screen_rect, TEMPLATE_TITLE, template_names, template_id)
    ...
    def fill_stacked_widget(self):
        # 第一个窗口，模板名称、模板说明输入框
        self.template_info_widget = QWidget(self.frame)
        ...
        # 第二个窗口，模板配置页
        self.config_widget = TemplateConfigWidget(self.parent_screen_rect)
        self.stacked_widget.addWidget(self.config_widget)

        # 第三个窗口，模板文件页
        self.template_file_widget = QWidget(self.frame)
    ...
    def setup_template_info_ui(self):
        ...
        # 模板说明文本输入框
        self.template_desc_label = QLabel(self.frame)
        self.template_desc_text_edit = TextEditor(self.frame)
        self.name_layout.addRow(self.template_desc_label, self.template_desc_text_edit)

    def setup_template_file_widget(self):
        # 构建模板文件页
        self.file_splitter = QSplitter(self.frame)
        # 不隐藏控件
    ...
    def open_create_file_dialog(self, dialog_title, current_file_name=None):
        self.file_name_check_dialog = SimpleNameCheckDialog(self.parent_screen_rect, dialog_title,
                                                            self.file_list_widget.collect_item_text_list(),
                                                            current_file_name)
        if current_file_name:
            self.file_name_check_dialog.edit_name_signal.connect(self.edit_file_name)
        else:
            self.file_name_check_dialog.add_name_signal.connect(self.add_template_file)
