...
from src.constant.template_dialog_constant import TEMPLATE_CONFIG_TITLE_TEXT, CONFIG_NAME_TEXT, VAR_NAME_TEXT, \
    VAR_NAME_PLACEHOLDER_TEXT, VAR_NAME_REG_RULE, WIDGET_LABEL_TEXT, CONFIG_DESC_TEXT, IS_REQUIRED_TEXT, \
    DEFAULT_VALUE_TEXT, PLACEHOLDER_TEXT, VALUE_RANGE_TEXT, ADD_VALUE_BTN_TEXT, CONFIG_INPUT_WIDGET_TYPE_DICT, \
    ADD_RANGE_VALUE_BOX_TITLE
...
class TemplateConfigDialog(NameCheckDialog):
    """模板配置信息对话框，不读取数据库数据，保存功能仅发送信号，不涉及数据库修改"""
    add_config_signal = pyqtSignal(TemplateConfig)
    edit_config_signal = pyqtSignal(TemplateConfig)
    ...
    def __init__(self, screen_rect, name_list, var_names, template_config=None):
        ...
        super().__init__(screen_rect, TEMPLATE_CONFIG_TITLE_TEXT, name_list, template_config, read_storage=False)
    ...
    def setup_other_content_ui(self):
        self.var_name_label = QLabel(self.frame)
        self.var_name_input = QLineEdit(self.frame)
        self.var_name_input.setObjectName('var_name_input')

        self.name_layout.addRow(self.var_name_label, self.var_name_input)

        self.var_name_checker = QLabel(self.frame)
    ...
    def open_save_range_value_dialog(self, dialog_title, range_value=None):
        self.save_range_value_dialog = SimpleNameCheckDialog(self.parent_screen_rect, dialog_title,
                                                             self.value_range_list_widget.collect_item_text_list(),
                                                             range_value)
        if range_value:
            self.save_range_value_dialog.edit_name_signal.connect(self.edit_combox_range_value)
        else:
            self.save_range_value_dialog.add_name_signal.connect(self.add_combox_range_value)
    ...
    def save_func(self):
        # 原数据存在，说明是编辑
        if self.dialog_data:
            self.edit_config_signal.emit(self.new_dialog_data)
        else:
            self.add_config_signal.emit(self.new_dialog_data)
        self.close()
