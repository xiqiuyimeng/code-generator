...
from src.constant.template_dialog_constant import TEMPLATE_FUNC_NAME_PLACEHOLDER_TEXT, EDIT_FUNC_BOX_TITLE, \
    CREATE_FUNC_BOX_TITLE
...
_author_ = 'luwt'
_date_ = '2023/3/28 9:58'


class TemplateFuncDetailDialog(NameCheckDialog):
    add_func_signal = pyqtSignal(TemplateFunc)
    edit_func_signal = pyqtSignal(TemplateFunc)
    ...
    def __init__(self, screen_rect, title, func_names, template_func=None):
        ...
        super().__init__(screen_rect, title, func_names, template_func, False)
    ...
    def setup_other_content_ui(self):
        self.text_editor = PyFuncEditor(self.frame)
    ...
    def add_post_process(self):
        self.add_func_signal.emit(self.new_dialog_data)
        self.close()

    def edit_post_process(self):
        self.edit_func_signal.emit(self.new_dialog_data)
        self.close()
