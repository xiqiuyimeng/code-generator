...
def add_insert_conn_tool(gui):
    # 指定图标
    insert_tool = QAction(QIcon(os.path.dirname(__file__) + '/../icon/add.jpg'), '添加连接', gui)
...
def add_refresh_tool(gui):
    refresh_tool = QAction(QIcon(os.path.dirname(__file__) + '/../icon/refresh.jpg'), '刷新', gui)
...
def add_generate_tool(gui):
    generate_tool = QAction(QIcon(os.path.dirname(__file__) + '/../icon/exec.jpg'), '生成', gui)
...
def add_exit_tool(gui):
    exit_tool = QAction(QIcon(os.path.dirname(__file__) + '/../icon/exit.jpg'), '退出程序', gui)
