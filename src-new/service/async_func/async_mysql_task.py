...
from constant_.constant import TEST_CONN_SUCCESS_PROMPT, TEST_CONN_FAIL_PROMPT, TEST_CONN_TITLE, OPEN_CONN_TITLE, \
...
class TestConnWorker(ThreadWorkerABC):
    ...
    def do_run(self):
        with DBExecutor(*self.connection[1:]) as executor:
            executor.test_conn()
            log.info(f'[{self.connection.name}]{TEST_CONN_SUCCESS_PROMPT}')
            self.success_signal.emit()

    def do_exception(self, e: Exception):
        log.error(f'[{self.connection.name}]{TEST_CONN_FAIL_PROMPT} --> {e}')
        self.error_signal.emit(f'[{self.connection.name}]{TEST_CONN_FAIL_PROMPT}\n{e}')


class TestConnLoadingMaskExecutor(LoadingMaskThreadExecutor):

    def __init__(self, connection: Connection, masked_widget, window):
        self.connection = connection
...
class OpenConnWorker(ThreadWorkerABC):
    ...
    def do_run(self):
        with DBExecutor(*self.connection[1:]) as executor:
            ...
            log.info(f'[{self.connection.name}]{OPEN_CONN_SUCCESS_PROMPT} ==> {dbs}')

    def do_exception(self, e: Exception):
        log.error(f'[{self.connection.name}]{OPEN_CONN_FAIL_PROMPT} --> {e}')
        self.error_signal.emit(f'[{self.connection.name}]{OPEN_CONN_FAIL_PROMPT}\n{e}')
...
class OpenDBWorker(ThreadWorkerABC):
    ...
    def __init__(self, connection: Connection, db_name):
        super().__init__()
        self.connection = connection
    ...
    def do_run(self):
        with DBExecutor(*self.connection[1:]) as executor:
            ...
            log.info(f'[{self.connection.name}][{self.db_name}]{OPEN_DB_SUCCESS_PROMPT} ==> {tables}')

    def do_exception(self, e: Exception):
        log.error(f'[{self.connection.name}][{self.db_name}]{OPEN_DB_FAIL_PROMPT} --> {e}')
        self.error_signal.emit(f'[{self.connection.name}][{self.db_name}]{OPEN_DB_FAIL_PROMPT}\n{e}')
...
class OpenTBWorker(ThreadWorkerABC):
    ...
    def __init__(self, connection: Connection, db_name, tb_name):
        super().__init__()
        self.connection = connection
        self.db_name = db_name
    ...
    def do_run(self):
        with DBExecutor(*self.connection[1:]) as executor:
            ...
            log.info(f'[{self.connection.name}][{self.db_name}][{self.tb_name}]{OPEN_TB_SUCCESS_PROMPT}'
                     f' ==> {columns}')
            self.success_signal.emit(columns)

    def do_exception(self, e: Exception):
        log.error(f'[{self.connection.name}][{self.db_name}][{self.tb_name}]{OPEN_TB_FAIL_PROMPT} --> {e}')
        self.error_signal.emit(f'[{self.connection.name}][{self.db_name}][{self.tb_name}]{OPEN_TB_FAIL_PROMPT}\n{e}')
